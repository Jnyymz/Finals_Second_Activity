...
function searchForAnApplicant($pdo, $keyword, $username) {
    $sql = "SELECT * FROM search_applicant WHERE 
            CONCAT(first_name, last_name, gender, age, email, phone_number, position, date_added) 
            LIKE ?";
    $stmt = $pdo->prepare($sql);
...
function insertNewApplicant($pdo, $first_name, $last_name,
    $gender, $age, $email, $phone_number, $position, $added_by) {
    ...
    $executeQuery = $stmt->execute([
        ...
        $gender, $age, $email, 
        $phone_number, $position, $added_by,
    ]);
    ...
    if ($executeQuery) {
		...
		$insertAnActivityLog = insertAnActivityLog($pdo, "INSERT", $getApplicantID['id'], 
			...
			$getApplicantID['gender'], $getApplicantID['age'], $getApplicantID['email'], $getApplicantID['phone_number'], 
            $getApplicantID['position'], $_SESSION['username']);
...
function editApplicant($pdo, $first_name, $last_name, $gender, $age, $email, 
    $phone_number, $position, $id, $modified_by) {
    ...
    $executeQuery = $stmt->execute([
        $first_name, $last_name, 
        $gender, $age, $email, 
        $phone_number, $position, $modified_by, $id
    ]);
    ...
    if ($executeQuery) {
		...
		$insertAnActivityLog = insertAnActivityLog($pdo, "UPDATE", $getApplicantID['id'], 
            ...
            $getApplicantID['phone_number'], $getApplicantID['position'], $_SESSION['username']);
...
function insertAnActivityLog($pdo, $operation, $id, $first_name, 
		$last_name, $gender, $age, $email, $phone_number, $position, $username) {

	$sql = "INSERT INTO activity_logs (operation, id, first_name, 
		last_name, gender, age, email, phone_number, position, username) VALUES(?,?,?,?,?,?,?,?,?,?)";
